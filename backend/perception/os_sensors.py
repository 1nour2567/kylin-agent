...
def _sanitize_log_line(line: str) -> str:
    """Remove control characters, truncate long lines — prevents indirect injection (#09)."""
...
class RealOSSensor(OSSensor):
    ...
    def get_disk(self) -> list:
        """Return list of disk info for all ext4/xfs/btrfs mount points (#01)."""
        out = self._run(["df", "-h", "--type=ext4", "--type=xfs", "--type=btrfs"])
        if not out:
            # Fallback to root only
            out_root = self._run(["df", "-h", "/"])
            if not out_root:
        ...
        return results if results else [{"filesystem": "?", "size": "?", "used": "?",
                "available": "?", "use_pct": "?", "mounted": "/"}]

    def get_large_files(self, min_size_mb: int = 50) -> list:
        """Scan /var/log, /tmp, /var/tmp, /home for files larger than min_size_mb (#01)."""
        cmd = [
            "find", "/var/log", "/tmp", "/var/tmp", "/home",
            "-type", "f", "-size", f"+{min_size_mb}M",
            "-printf", "%s %p %u %g %M %Ax %Cx\\n",
        ]
        out = self._run(cmd)
        if not out:
            return []
    ...
    def get_journalctl_logs(self, unit: str = "", lines: int = 50) -> dict:
        ...
        for line in out.split("\n")[:lines]:
            if line.strip():
                entries.append(_sanitize_log_line(line[:300]))
    ...
    def get_io_stats(self) -> dict:
        """Read /proc/diskstats for disk I/O statistics (#18). No iostat dependency."""
        try:
            with open("/proc/diskstats") as f:
                lines = f.readlines()
        except Exception:
            return {"devices": [], "note": "/proc/diskstats unavailable"}
        devices = []
        for line in lines:
            parts = line.split()
            if len(parts) >= 14:
                devices.append({
                    ...
                    "reads": int(parts[3]),
                    "writes": int(parts[7]),
                    "read_mb": round(int(parts[5]) * 512 / 1048576, 1),
                    "write_mb": round(int(parts[9]) * 512 / 1048576, 1),
                    "io_time_ms": int(parts[12]),
                })
        return {"devices": devices[:10]}

    def get_config_drift(self) -> list:
        """Check rpm -Va for config file changes (#18)."""
        out = self._run(["rpm", "-Va", "--nomtime", "--nouser", "--nogroup"])
        if not out:
            return []
        changes = []
        for line in out.split("\n")[:30]:
            if line.strip() and len(line) >= 2:
                changes.append({"flags": line[:9], "path": line[10:].strip()})
        return changes
...
class MockOSSensor(OSSensor):
    ...
    def get_large_files(self, min_size_mb: int = 50) -> list:
        return [
            {"size_bytes": 104857600, "path": "/var/log/mysql/slow-query.log",
             "owner": "mysql", "group": "mysql", "mode": "-rw-r--r--",
             "atime": "2026-07-14", "ctime": "2026-07-10"},
            {"size_bytes": 52428800, "path": "/tmp/nginx_error.log.old",
             "owner": "root", "group": "root", "mode": "-rw-r--r--",
             "atime": "2026-07-01", "ctime": "2026-06-15"},
        ]

    def get_disk(self) -> list:
        return [
            {"filesystem": "/dev/sda1", "size": "50G", "used": "32G",
             "available": "15G", "use_pct": "68%", "mounted": "/"},
            {"filesystem": "/dev/sda2", "size": "100G", "used": "89G",
             "available": "8G", "use_pct": "92%", "mounted": "/var"},
        ]
    ...
    def get_loadavg(self) -> dict:
        return {"load1": 1.2, "load5": 0.9, "load15": 0.7}

    def get_io_stats(self) -> dict:
        return {"devices": [
            {"device": "sda", "reads": 42000, "writes": 18000,
             "read_mb": 328.1, "write_mb": 140.6, "io_time_ms": 5200},
            {"device": "sdb", "reads": 1200, "writes": 89000,
             "read_mb": 9.4, "write_mb": 695.3, "io_time_ms": 18400},
        ]}

    def get_config_drift(self) -> list:
        return [
            {"flags": "S.5....T.", "path": "/etc/ssh/sshd_config"},
            {"flags": "..5....T.", "path": "/etc/nginx/nginx.conf"},
        ]
