...
Integrated with FileKnowledgeBase for semantic file classification (v2.1).
...
class ConstraintEngine:
    ...
    # ── Semantic file type recognition (赛题: "识别是否为关键数据库日志") ──
    # Files matching these patterns are flagged as critical before deletion/truncation
    CRITICAL_FILE_PATTERNS = [
        # Database files
        (r"\.(db|sql|sqlite|sqlite3|mdb|mdf|ldf|dbf|frm|ibd|myi|myd)$", "Database file"),
        (r"(mysql|postgres|mongodb|redis|etcd|consul)/data", "Database data directory"),
        # Database logs
        (r"(mysql|mariadb|postgres|mongod|redis)\.log", "Database server log"),
        (r"(slow|error|general)-query\.log", "Database diagnostic log"),
        (r"pg_log|pg_xlog|pg_wal", "PostgreSQL WAL/log directory"),
        # System logs (critical for audit)
        (r"/var/log/(secure|messages|syslog|kern\.log|auth\.log|audit/)", "System security/audit log"),
        (r"systemd.*\.journal", "Systemd journal file"),
        # Configuration state
        (r"\.(conf|cfg|ini|yaml|yml|toml)$", "Configuration file"),
        (r"nginx/.*\.conf", "Nginx configuration"),
        (r"(crontab|anacrontab)", "Scheduled task configuration"),
        # Binary/executable
        (r"/usr/(bin|sbin|local/bin)/", "System executable"),
        (r"\.(so|ko|o)$", "Shared library / kernel object"),
    ]

    # Critical directories that should never have files blindly deleted
    CRITICAL_DIRECTORIES = [
        "/var/lib/mysql", "/var/lib/postgresql", "/var/lib/mongodb",
        "/var/lib/redis", "/etc/mysql", "/etc/postgresql",
        "/var/log", "/var/spool/cron", "/var/spool/anacron",
        "/opt", "/usr/local",
    ]
    ...
    # ── Public wrappers for TOCTOU re-check (#review-5) ──

    def check_file_path(self, path: str) -> Optional[ValidationResult]:
        """Public API: validate a file path for blocked prefixes."""
        return self._check_file_path(path)

    def check_critical_file(self, path: str) -> Optional[ValidationResult]:
        """Public API: classify file for semantic risk."""
        return self._check_critical_file(path)

    # ── Private helpers ──

    def _check_file_path(self, path: str) -> Optional[ValidationResult]:
        """Validate a file path for create/append/delete operations."""
        import os as _os
        if not path:
            return ValidationResult(allowed=False, reason="File path is required", risk_score=10)
        abs_path = _os.path.realpath(_os.path.abspath(_os.path.expanduser(path)))
    ...
    def _check_critical_file(self, path: str) -> Optional[ValidationResult]:
        """Semantic file type recognition via FileKnowledgeBase.

        Competition requirement A2: "识别该文件是否为关键数据库日志".
        Distinguishes: temp cache (safe to clean) vs expired logs (safe) vs
        database logs (confirm) vs system audit logs (dangerous) vs config/data (never).
        """
        import os as _os
        if not path:
            return None

        # Cross-platform representation of the Linux demo's low-risk /tmp
        # workspace.  Windows test hosts normalize /tmp to e.g. C:\tmp.
        normalized_path = str(path).replace("\\", "/").lower()
        if (
            normalized_path == "/tmp"
            or normalized_path.startswith("/tmp/")
            or re.match(r"^[a-z]:/tmp(?:/|$)", normalized_path)
        ):
            return None

        from security.file_knowledge import FileKnowledgeBase
        classification = FileKnowledgeBase.classify(path)

        if classification.risk_score <= 2:
            # Safe to clean — temp cache, expired logs, package cache
            return None  # Allow without confirmation

        if classification.risk_score <= 3:
            # Core dumps — auto but with a note
            return ValidationResult(
                allowed=True,
        ...
        if classification.risk_score <= 5:
            # Application logs / user uploads — confirm
            return ValidationResult(
                allowed=True,
        ...
        # Risk 7+: database logs, system logs, configs, data — confirm or veto
        return ValidationResult(
            allowed=classification.risk_score < 9,  # veto at 9+
            requires_confirmation=classification.risk_score < 9,
            reason=classification.reason,
            alternative=classification.suggestion,
            risk_score=classification.risk_score,
        )
    ...
    def _validate_tool_call(self, tool_name: str, params: dict,
                            ...
                            intent_profile: dict = None) -> ValidationResult:
        # 0. Normalize file path params: file/filename/target → path (#02)
        params = dict(params or {})
        if "path" not in params:
            for alias in ("file", "filename", "target"):
                if alias in params:
                    params["path"] = params.pop(alias)
                    break

        # 0.1 Intent profile risk boost: escalate threshold on suspicious signals
        intent_boost = 0
        if intent_profile:
        ...
        if params and "path" in params:
            ...
            if path:
                ...
                if result and not result.allowed:
                    return result
                # Semantic file type check: identify critical database logs, configs
                critical = self._check_critical_file(path)
                if critical and not critical.allowed:
                    return critical  # Veto immediately (config_file / database_data / binary_library)
                # Store confirm-level classification for later
                if critical and critical.requires_confirmation:
                    pending_confirmation = critical
                # Confirm-tier for system paths
                _os_mod = __import__("os")
                abs_path = _os_mod.path.realpath(
                    _os_mod.path.abspath(_os_mod.path.expanduser(path))
                )
        ...
        if params and tool_name in ("bash", "execute_script") and "path" in params:
            path = str(params.get("path", ""))
            if path:
        ...
        # 0b. Param-level semantic validation for high-risk tools (#03)
        param_validation = self._validate_params(tool_name, params)
        if param_validation and not param_validation.allowed:
            return param_validation

        # 1. Viewer role can only use manifest-declared read-only tools.
        from agent.tools_manifest import lookup_by_llm_name, lookup_by_mcp_name
        manifest_entry = lookup_by_llm_name(tool_name) or lookup_by_mcp_name(tool_name)
        viewer_allowed = bool(manifest_entry and manifest_entry.get("risk") == "readonly")
        if role == "viewer" and not viewer_allowed:
            return ValidationResult(
                allowed=False,
        ...
        # Return pending confirmation from file classification if nothing else matched
        if pending_confirmation is not None:
            return pending_confirmation

        return ValidationResult(allowed=True, risk_score=1)

    def _validate_params(self, tool_name: str, params: dict) -> Optional[ValidationResult]:
        """Semantic parameter validation for high-risk tools (#03)."""
        import re

        # kill_process: PID must be positive integer in valid range
        if tool_name in ("kill_process", "kill"):
            pid_str = str(params.get("pid", params.get("target", "")))
            try:
                pid = int(pid_str)
                if pid < 1 or pid > 4194304:
                    return ValidationResult(
                        allowed=False, risk_score=10,
                        reason=f"PID {pid} out of valid range (1-4194304)",
                        alternative="Specify a valid PID from ps_processes output",
                    )
                if pid == 1:
                    return ValidationResult(
                        allowed=False, risk_score=10,
                        reason="Cannot kill PID 1 (init/systemd)",
                        alternative="Use systemctl to manage the init system",
                    )
            except (ValueError, TypeError):
                if pid_str.strip():
                    return ValidationResult(
                        allowed=False, risk_score=10,
                        reason=f"Invalid PID: '{pid_str}' — must be a positive integer",
                        alternative="Use ps_processes to find the correct PID",
                    )

        # systemctl restart/stop: validate service name format
        if tool_name in ("systemctl_restart",):
            service = str(params.get("service", params.get("unit", "")))
            if not re.match(r'^[a-zA-Z][a-zA-Z0-9_\-\.@]*$', service):
                return ValidationResult(
                    allowed=False, risk_score=10,
                    reason=f"Invalid service name format: '{service}'",
                    alternative="Use get_services to see valid service names",
                )
            # Block attempts to restart critical infrastructure
            critical_services = {"systemd-init", "dbus", "polkit"}
            if service.lower() in critical_services:
                return ValidationResult(
                    allowed=False, risk_score=10,
                    reason=f"Cannot restart critical system service: {service}",
                    alternative="This service requires physical console access",
                )

        # execute_script: validate path under allowed directory
        if tool_name == "execute_script":
            path = str(params.get("path", ""))
            if path and not path.startswith("/tmp/kylin-agent/"):
                return ValidationResult(
                    allowed=False, risk_score=10,
                    reason=f"Script path must be under /tmp/kylin-agent/, got: {path}",
                    alternative="Copy the script to /tmp/kylin-agent/ first",
                )
