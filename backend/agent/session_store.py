...
class SessionStore:
    ...
    def get(self, session_id: str, user_id: str = "") -> Optional[dict]:
        self._maybe_cleanup()
        with self._lock:
            s = self._sessions.get(session_id)
            # Ownership check (#08): must match user_id unless empty string
            if s and user_id and s.get("user_id") != user_id:
                return None
            if s:
                s["last_access"] = time.time()
            return s
