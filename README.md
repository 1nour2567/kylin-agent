# 麒盾智维 KylinShield Ops

> **智察系统，安全运维** — A Secure Intelligent Operations Agent for Kylin OS Based on MCP and Multi-Layer Guardrails
...
| — | `security/tool_gateway.py` | Unified MCP tool-call security gate: role check, T2/T3 validation, confirmation queue, audit |
| — | `security/redaction.py` | Redacts API keys, Bearer tokens, passwords, and secrets before logs/audit/UI pushes |
...
- MCP `tools/call` now enters the same backend guardrail path as `/api/chat`: authenticated identity, role authorization, deterministic T2/T3 validation, confirmation queue, and audit chain.
- Non-readonly MCP tools require explicit human confirmation even when invoked directly through JSON-RPC.
- Service defaults are local-only: `HOST=127.0.0.1`, restricted CORS, and optional `TLS_CERTFILE`/`TLS_KEYFILE` or `ENFORCE_HTTPS=true` for production HTTPS.
- API keys are not stored in frontend `localStorage`; logs and audit events redact common token and secret formats.
- Confirm-tier commands no longer imply root execution. Privileged sudo is disabled by default and can only be enabled with `ALLOW_PRIVILEGED_CONFIRM=true` plus a narrow sudoers allowlist.
...
Default bind address is `127.0.0.1`. Set `HOST=0.0.0.0` only behind firewall/auth controls, preferably behind an HTTPS reverse proxy.
...
| `/api/mcp/tools` | GET | Bearer | Registered MCP tools |
...
```bash
cd backend
python -m pytest tests/ -v          # 141 tests, all structural
```
...
The install script creates a `kylin-agent` service user and a systemd unit with `User=kylin-agent`.
Do not grant broad `NOPASSWD`. If a demo requires privileged confirm-tier actions, enable `ALLOW_PRIVILEGED_CONFIRM=true` and add only the exact command patterns required by the runbook.

## Task-Level CapBAC and tool-output IPI defense

All state-changing tools use a short-lived, single-use HMAC-SHA256 task
capability. The scope binds the authenticated user and role, canonical tool,
resource, normalized parameter SHA-256, trace ID, operation ID, event ID,
expiry, and nonce. Confirmation-tier capabilities are issued only after
`POST /api/confirm`; automatic low-risk writes receive an internal token that
is immediately verified and consumed. This is Task-Level CapBAC with
context-bound scope, not a complete Macaroons implementation.

External system data, tool stdout/stderr, MCP tool results, and MCP resources
are recursively inspected before they reach the UI or Reasoner. Suspect
instruction-like content is replaced with `[IPI_CONTENT_REMOVED ref=...]`;
high-confidence results terminate the agentic loop and tighten risk posture.

Production requires a dedicated `CAPABILITY_TOKEN_SECRET` and fails closed when
it is missing. Development/test creates a process-local key with a prominent
warning. See `backend/.env.example` for all CapBAC and IPI settings.
